 ...
 * @author C6H2Cl2
 */
class CompositeBow :ItemBow(){
    init {
        creativeTab = ReinforcedToolsRegistry.tabReinforcedTools
        unlocalizedName = "${ReinforcedToolsCore.Domain}.compositeBow"
        setTextureName("${ReinforcedToolsCore.Domain}:compositeBow")
        maxDamage = 2048
        iconString = "${ReinforcedToolsCore.Domain}:compositebow"
    }
    ...
    override fun getMaxItemUseDuration(itemStack: ItemStack?): Int {
        ...
        return if(itemStack.tagCompound.getBoolean("auto_shot")) 32 else 72000
    }
    ...
    override fun getIcon(stack: ItemStack, renderPass: Int, player: EntityPlayer?, usingItem: ItemStack?, useRemaining: Int): IIcon {
        ...
        return if(useTick < 14) iconArray[0] as IIcon else if (useTick < 20) iconArray[1] as IIcon else  iconArray[2] as IIcon
    }
}