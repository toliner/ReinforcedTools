 ...
 * @author C6H2Cl2
 */
class ItemExcavationHammer(material: ToolMaterial,private val level: Int): ItemTool(-2f, material, null), IReinforcedTools {
    ...
    init {
        ...
        unlocalizedName = "${name[0].toLowerCase()}${name.substring(1)}ExcavationHammer"
        setTextureName("$MOD_ID:${name.toLowerCase()}_excavation_hammer")
    ...
    override fun getToolClasses(stack: ItemStack?): MutableSet<String> {
        return mutableSetOf("pickaxe", "hammer", "Pickaxe", "Hammer")
    }

    override fun canHarvestBlock(block: Block, itemStack: ItemStack?): Boolean {
        return (0 <= block.getHarvestLevel(0) && block.getHarvestLevel(0) <= toolMaterial.harvestLevel && getToolClasses(itemStack).contains(block.getHarvestTool(0)))
    }
