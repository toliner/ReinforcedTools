 ...
 * @author C6H2Cl2
 */
class ItemShortBow : ItemBow() {
    init {
        creativeTab = ReinforcedToolsRegistry.tabReinforcedTools
        unlocalizedName = "$MOD_ID.shortBow"
        setTextureName("$MOD_ID:short_bow")
        maxDamage = 2048
        //@JvmField
        //@SideOnly(Side.CLIENT)
        iconString = "$MOD_ID:short_bow"
    }
    ...
    fun shotArrow(itemStack: ItemStack, world: World, player: EntityPlayer, useTick: Int) {
        ...
        //チャージ時間測定
        j = event.charge
        player.addChatComponentMessage(ChatComponentText(event.charge.toString()))
